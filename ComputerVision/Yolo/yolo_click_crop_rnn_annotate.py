...
def marker(event, x, y, flags, param):
	...
	global stop
	global key
	global last_deleted_position

	if event == cv2.EVENT_LBUTTONDBLCLK:
		center = (x,y)
		# markers.append([x, y])
		markers.insert(last_deleted_position,[x, y])
		last_deleted_position = last_deleted_position + 1

		# radius = 5
		# for point_i in markers:
		# 	cv2.circle(param, tuple(point_i), radius, (0,0,255), 2)
		# isClosed = False
		# thickness = 1
		# color = (0, 255, 0)
	...
	if event == cv2.EVENT_RBUTTONDBLCLK:
		p = [x,y]
		xy = np.array(markers).T
		d = ((xy[0] - p[0]) ** 2 + (xy[1] - p[1]) ** 2) ** 0.5
		closest_idx = np.argmin(d)
		last_deleted_position = closest_idx
		del markers[closest_idx]
...
cv2.namedWindow("dot")
# function called when mouse events occur
...
while key!=27:
	...
	if len(markers)>0:
		radius = 5
		for point_i in markers:
			cv2.circle(roi, tuple(point_i), radius, (0,0,255), 2)
		isClosed = False
		thickness = 1
		color = (0, 255, 0)
		cv2.polylines(roi,[np.array(markers)],isClosed, color,thickness)
		print(markers)
...
if run_rnn==1:
	...
	cv2.namedWindow("RNN")
	# function called when mouse events occur
	cv2.setMouseCallback("RNN", marker, param=roi)
...
# print(markers)
	...
	##C:/Users/salmansaeed.khan/.conda/envs/PolyRNN/python.exe d:/Personal-GIT/mlpractise/ComputerVision/Yolo/yolo_click_crop_rnn_annotate.py --image image_0.jpg --yolo yolo-coco
	##C:/Users/salmansaeed.khan/.conda/envs/PolyRNN/python.exe d:/Personal-GIT/mlpractise/ComputerVision/Yolo/yolo_click_crop_rnn_annotate.py --image dog.jpg --yolo yolo-coco
